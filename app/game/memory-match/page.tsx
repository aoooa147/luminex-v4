...
export default function MemoryMatchPage() {
  ...
  async function handleClaimReward() {
    ...
    try {
      ...
      console.log('📤 Sending init request:', { address, gameId: GAME_ID, amount: rewardAmount });
      const initRes = await fetch('/api/game/reward/init', {
        ...
        body: JSON.stringify({ 
          address, 
          gameId: GAME_ID, 
          amount: rewardAmount // Ensure it's a number
        })
      });
      ...
      if (!initRes.ok) {
        console.error('❌ Init API failed with status:', initRes.status, initData);
        alert(initData.error || initData.message || 'Failed to initialize reward transaction. Please try again.');
      ...
      console.log('✅ Init API succeeded:', { reference: initData.reference, amount: initData.amount });
